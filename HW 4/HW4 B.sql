create table Loan (
    ...
    Loan_Title varchar2(100),
    Risk_Score int,
    Debt_To_Income_Ratio float(10),
    ZipCode int, State varchar2(100),
...
insert into Loan values (8000, '5/28/2007', 'Recent College Grad Wants to Pay OffCCs',   708, 10, 37427, 'TN', 1, 0);
...
insert into Loan values (15000, '5/29/2007', 'Established borrower consolodatingdebt',   698, 15.56, 55927, 'MN', 1, 0);
...
insert into Loan values (2100, '6/1/2007', 'A clean sweep before buying a newhouse', 684, 10, 20827, 'MD', 1, 0);
...
--i
WITH Histogram (ris_score, count) AS (
    SELECT risk_score, COUNT(risk_score) AS count FROM loan
    GROUP BY risk_score
    ORDER BY COUNT DESC 
)
...
--ii
WITH RiskScoresWithMinLoans (risk_score, count) AS (
    SELECT risk_score, COUNT(risk_score) AS count FROM loan
    GROUP BY risk_score
    ORDER BY risk_score DESC
)

SELECT risk_score, COUNT FROM RiskScoresWithMinLoans
WHERE COUNT < 2;



--iii
WITH RiskScoresWithMinLoans (amount, loan_date, loan_title, risk_score, debt_to_incom_ratio, zipcode, state, employment_length, policy_code, count)
AS (
    SELECT amount, loan_date, loan_title, risk_score, debt_to_income_ratio, zipcode, STATE, employment_length, policy_code, COUNT(risk_score) AS count FROM loan
    GROUP BY amount, loan_date, loan_title, risk_score, debt_to_income_ratio, zipcode, STATE, employment_length, policy_code
)
