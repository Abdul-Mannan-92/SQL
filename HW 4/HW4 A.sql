...
--i
SELECT SUM (timetofeed) AS TotalTimeToFeed
FROM Animal
...
WHERE TimetoFeed > (SELECT MAX(TimeToFeed) FROM Animal WHERE ACategory = 'rare');
-- NO ANIMAL HAVE LARGER TIME TO FEED THAN RARE ANIMALS.
...
WHERE remuneration = (
    SELECT MAX(MAX(hourlyrate*timetofeed)) AS remuneration
    FROM zookeeper, handles, animal
    WHERE zookeepid = zid AND animalid = aid
    GROUP BY Zname
);
