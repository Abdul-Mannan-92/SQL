...
--(2)
SELECT dno, dname, AVG (SALARY) FROM department, employee
WHERE dno = dnumber
GROUP BY dname,dno;
...
--(5)
SELECT dname Department_name, COUNT(DISTINCT ssn) total_no_of_employees,
COUNT(dependent_name) total_no_of_dependents 
FROM department d,employee e LEFT OUTER JOIN DEPENDENT dp ON e.ssn = DP.essn 
WHERE e.dno = d.dnumber GROUP BY dname,dnumber;


--(6)
--Male employee's avg(37000) earns more than Female employee's avg(28625){from disscussion}  
SELECT AVG(SALARY)
...
--(7)
SELECT Fname,Lname FROM employee
WHERE salary > (SELECT MAX(salary)-20000 FROM employee) AND salary < (SELECT MAX(salary) FROM employee)
