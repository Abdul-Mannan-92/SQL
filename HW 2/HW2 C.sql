...
SELECT MIN(Fare), TravelMode
FROM Trips GROUP BY TravelMode;
...
FROM Trips T1 
JOIN Trips T2 ON T1.TripState = T2.TripState AND T1.TID != T2.TID 
WHERE T1.TravelMode = 'Car' AND T2.TravelMode = 'Train' AND T1.Fare > T2.Fare;
    

    
SELECT DISTINCT T1.TID, T2.TID 
FROM ByCar T1 JOIN ByCar T2 ON T1.Mileage = T2.Mileage AND T1.TID < T2.TID;
    

    
SELECT DISTINCT T1.TID, T2.TID 
FROM ByTrain T1 JOIN ByTrain T2 ON T1.TrainSpeed < T2.TrainSpeed ;
   

    
SELECT DISTINCT T1.TID, T2.TID 
FROM Trips T1 JOIN Trips T2 ON T1.TripState = T2.TripState AND T1.TravelMode = T2.TravelMode AND T1.TID < T2.TID;
...
INNER JOIN Trips T3 ON T1.TripState = T3.TripState
WHERE T1.TravelMode = 'Car' AND T2.TravelMode = 'Train' AND T3.TravelMode = 'Plane';